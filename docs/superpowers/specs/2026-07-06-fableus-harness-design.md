...
| G-4 | 통과 기준 | 확정 blocking 0건. 1인 단독 blocking·discretionary는 정보성 표시만 |
...
| G-8 | 수렴 상한 | 라운드 상한 없음 — 확정 blocking 0까지 반복. 매 라운드 사용자 환류가 체크포인트(발산 판단·중단은 사용자 몫). D-9로 대체(구 "최대 3라운드") |
| G-9 | 리더 모델 | 일반 파라미터, 기본 `opus`. 특정 모델 전용 기능 없음 |
| G-10 | 3-리더 합의 투표 | 콜드 리드는 독립 리더 N명 병렬(`--readers` 기본 3, `N<2` 거부). 2인 이상이 같은 지점을 blocking으로 제기한 것만 확정 blocking. 출처: 2026-07-07 도그푸딩 게이트 R2~R4 사용자 채택 (D-8) |
| G-11 | 기계 린트 바닥 검사 | 콜드 리드 전 재현 가능한 바닥. 하드폴 2조건(필수 `*(mandatory)*` 섹션 공백, `[NEEDS CLARIFICATION:` 마커 ≥1)만 반려, 문체 검사(모호 형용사·관찰 가능성)는 경고만. 린트는 통과 판정을 대체하지 않음. 출처: 2026-07-07 도그푸딩 게이트 R2~R4 사용자 채택 (D-8·D-11) |
| G-12 | "같은 지점" 정의 | 앵커 일치 AND 논지 일치. 리더가 이슈마다 앵커(FR/섹션) 필수 태깅, 동일 앵커·상이 논지는 분리 집계. 집계용 LLM 심판 금지. 출처: 2026-07-07 도그푸딩 게이트 R2~R4 사용자 채택 (D-10·D-12) |
