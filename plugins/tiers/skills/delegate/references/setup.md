...
3. **small_edit_chars** — the largest single Edit the main session may still make while delegate mode is on: 200 (Recommended) / 0 (none) / 500. Mention that other values can be typed via Other.

Then:

1. Write the result to `${CLAUDE_PLUGIN_DATA}/delegate.json` (create the directory first if needed), keeping `enabled` and `log` as they were.
