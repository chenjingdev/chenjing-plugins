---
name: delegate
description: Use when the user runs /tiers:delegate — the switch for the tiers delegate hook. `on`/`off` turns the whole hook on or off (model pinning, code-edit denial in the main session, prose-edit denial for the worker, brief check), `status` shows config and recent handoffs, `setup` changes the tier, `<task>` turns it on and does the task. The working rules arrive from the hook's deny messages and the worker's replies, not from this skill.
...
| anything else | TASK = the arguments. If `enabled` is false, set it to true first and say so. Then do TASK: code goes through `tiers:worker`, prose you write yourself; when you reach for the wrong tool the hook says what to do instead. |
