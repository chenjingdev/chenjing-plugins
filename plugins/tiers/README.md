...
세션이 계획하고 worker가 구현하고 세션이 검증한다. 글은 반대다. 사람이 읽을 문서는 세션이 직접 쓰고 worker는 손대지 않는다. 설치만 해서는 훅이 아무 일도 하지 않는다. `on`을 켠 사람의 세션에서만 돈다. 켜진 세션은 시작할 때 역할문 한 장을 받는다. 너는 판단하고 브리프하고 검증하는 쪽이다, 코드는 worker, 넓은 조회는 scout, 화면과 실행 확인도 worker. 스킬을 부르지 않은 세션에서도 훅은 돌고, 막힐 때 거부문이 다음 할 일을 알려준다. worker를 부르는 법, 브리프 양식, BLOCKED에 답하는 법이 전부 그 두 길로 온다.

```
/tiers:delegate on          → 훅을 켠다. 세션 시작 시 역할문, 모델 고정, 메인 세션의 코드 편집 거부, worker의 문서 편집 거부
...
/tiers:delegate <작업>       → 꺼져 있으면 켜고, 그 작업을 한다
```

### 훅이 하는 일

`hooks/guard.js` 하나가 SessionStart, PreToolUse, SubagentStop에 붙는다. 설정 파일은 호출마다 다시 읽으니 바꾸면 바로 먹는다. 스위치는 `enabled` 하나다. 켜면 아래가 한꺼번에 돌고, 끄면 모델 고정까지 전부 멈춘다.

- **역할문**: 세션이 시작·재개되거나 컴팩션이 일어나면 `skills/delegate/references/role.md`를 그대로 컨텍스트에 넣는다. 세션이 첫 도구를 고르기 전에 자기 자리를 안다. 규칙 본문과 효력 범위는 그 파일 하나에만 있고, 이 문서도 스킬도 그 파일을 가리키기만 한다. 꺼져 있으면 넣지 않는다. 세션 중간에 `on`을 켰으면 스킬이 같은 파일을 따르라고 한다.
...
거부 사유는 짧다. 편집을 막을 때는 두 줄이다. 여기서는 못 고친다, `tiers:worker`에 넘겨라, 끄려면 `/tiers:delegate off`. worker가 문서에 손대다 막힐 때도 두 줄이다. 파일은 두고 보고에 적어 넘겨라. 브리프 템플릿은 네 절이 빠진 브리프를 거부할 때만 따라온다. 템플릿이 필요한 순간이 거기다.

역할문과 거부문은 서로 다른 구멍을 막는다. 0.3.x는 거부문만 두었다. 규칙을 세션 프롬프트에 깔아 두면 희석된다고 봤다. 그런데 거부문은 쓰기에서만 뜬다. 조회는 훅에 닿지 않으니 세션은 저장소를 다 읽고 스크린샷까지 본 뒤에야 처음으로 "네 일이 아니다"를 들었고, 그때는 비싼 부분이 이미 끝나 있었다. 7월에 전역 CLAUDE.md에 두었던 Advisor/Worker 규칙이 더 잘 먹혔던 이유가 여기다. 세션이 첫 도구를 고르기 전에 자리를 알았다. 그 규칙은 항상 켜져 있어서 끄고 싶을 때 못 껐고, 그래서 지웠다. 0.4.0은 같은 글을 훅으로 넣는다. 스위치를 따라 켜지고 꺼지며, 쓰기는 거부문이 여전히 뒷받침한다.
...
- 넓은 수색은 scout. 발췌와 출처를 그대로 가져오고 결론은 내지 않는다. 판단에 바로 들어가는 좁은 읽기는 세션이 직접 한다. 첫 Read나 grep 전에 한 번 묻는다. 이게 브리프에 들어가는 읽기인가, worker가 할 탐색을 내가 하고 있는가.
- 앱을 띄우고, 브라우저를 몰고, 스크린샷을 찍어 보고, 긴 테스트를 돌리는 일은 worker. 세션은 숫자와 사실을 받는다. 스크린샷과 로그 덤프는 세션 컨텍스트에 남아 매 턴 다시 읽힌다.
...
| `enabled` | `true` `false` | 훅 전체. 세션 시작 시 역할문, 모델 고정, 메인 세션 코드 편집 차단, worker 산문 차단, 브리프 검사 |
