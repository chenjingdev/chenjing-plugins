...
세션이 계획하고 worker가 구현하고 세션이 검증한다. 글은 반대다. 사람이 읽을 문서는 세션이 직접 쓰고 worker는 손대지 않는다. 설치만 해서는 훅이 아무 일도 하지 않는다. `on`을 켠 사람의 세션에서만 돈다. 스킬을 부르지 않은 세션에서도 훅은 돌고, 막힐 때 거부문이 다음 할 일을 알려준다. worker를 부르는 법, 브리프 양식, BLOCKED에 답하는 법이 전부 그 길로 온다.
