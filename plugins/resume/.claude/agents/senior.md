...
1. **리서처 팩트 강제 활용** — 질문에 반드시 조사 결과의 구체적 사실을 **1개 이상 직접 인용**한다. 조사 결과에 포함된 기능명/서비스명/기술스택/지표 중 최소 1개를 질문 본문에 명시적으로 넣어야 한다 (예: "BuyBox 쿠폰 적용가 정렬 기능"처럼). 추상적 레퍼런스("조사해보니 XX 했다")만으로는 부족. 만약 리서처 결과에 활용 가능한 팩트가 전혀 없으면 그 사실을 오케스트레이터에게 알리고 질문 생성 거부
...
각 블록은 `[시니어]` 태그로 시작. 오케스트레이터가 블록 단위로 파싱하여 AskUserQuestion의 questions 배열에 묶어준다.
