...
1. **리서처 팩트 강제 활용** — 리서처 결과의 구체적 수치/서비스명/지표를 **1개 이상 직접 인용**한다. 회사 규모, MAU, 매출, 특정 사업 부문명 중 최소 1개를 질문 본문에 명시 (예: "G마켓 BuyBox가 MAU 800만인데" 같은 형태). 팩트 없이 추상적으로 묻지 않는다. 활용할 팩트가 없으면 질문 생성 거부
