...
앱 이름은 짧은 소문자 식별자로 정한다(`myapp` → `myappbench`). 사용자가 부른 이름을 따르되 셸 함수 이름이 되므로
...
- **벤치가 개인 데이터를 건드리는가** → 서버가 사용자의 실사용 스토어(기억·라이브러리)에 쓰기를 하면, 벤치 런이 그 데이터를
  ...
  넣는다(`--data-dir`, `ARIA_DATA_DIR` 같은 그 앱의 수단). 값에 `{harness}`를 넣으면 하네스별로 갈라져 서로 다른 하네스를 동시에 돌릴 수 있다(runner.md). 반대로 회상·검색 벤치처럼 실데이터가 있어야 의미 있는 앱은 읽기
  전용인지 확인하고 실데이터를 쓴다 — 어느 쪽이든 이유를 보고에 적는다.
- **새 이름을 지을 때는 먼저 존재 여부를 확인한다.** 데이터 디렉터리·peer·네임스페이스처럼 "벤치용으로 하나 새로" 정하는
  이름은 `ls`·API 조회로 비어 있는지 확인한 뒤 쓴다. 기존 작업 공간과 충돌하면 과거 산출물을 섞거나 덮을 수 있고,
  존재하지 않는 peer 이름을 기본값으로 넣으면 관련 도구가 전부 깨질 수 있다.
- **서버 안내문(instructions)이나 도구 설명이 스킬을 언급하는가** → "○○ 스킬을 먼저 불러라" 한 줄만 있어도 raw 실행의 모델은
  `ls ~/.claude/skills`로 실제 홈을 뒤져 그 스킬을 셸로 읽어 버린다(aria가 그랬다). 프로필 격리는 CLI 설정만 가르고 파일시스템은
  공유라 러너가 막을 수 없다. `init`이 이 문구를 찾으면 경고를 내니, 경고가 나오면 서버 소스에서 그 문구를 지우고(스킬이
  설치돼 있으면 하네스가 어차피 목록에 띄운다) `init <app> --refresh-tools`로 사라졌는지 확인한다. 서버를 못 고치는 앱이면
  raw 트랙이 오염될 수 있다고 보고에 적는다.
