...
//   bench run <app> <harness> [--effort E] [--model M] [--dry-run] [-- 프롬프트…]
//   bench skill <app> on|off              앱의 스킬을 벤치 프로필에 설치/제거 (기본 미설치 = raw). 상태는 status 에 보인다
...
function skillInstalled(app, a) {
  ...
  return skillSlots(app).every((slot) => a.skills.every((s) => exists(path.join(slot, path.basename(s)))));
}
function setSkill(app, a, on) {
  if (!a.skills.length) throw new Error(`${app}: 등록된 스킬이 없습니다(앱 JSON 의 skills 가 비어 있음)`);
  for (const slot of skillSlots(app)) {
    for (const s of a.skills) {
      const at = path.join(slot, path.basename(s));
      if (on) link(s, at); else { try { fs.rmSync(at, { recursive: true, force: true }); } catch {} }
    }
  }
}
...
function initAgy(app, a) {
  const home = dirs(app).agy;
...
async function init(app, opts) {
  ...
  for (const old of ["codex-raw", "codex-skill", "agy-raw", "agy-skill", path.join("work", "raw"), path.join("work", "skill")]) {   // 0.1.x 배치 정리
    try { fs.rmSync(path.join(dirs(app).app, old), { recursive: true, force: true }); } catch {}
  }
  initClaude(app, a);
  initCodex(app, a);
  initAgy(app, a);
...
function parseRun(argv) {
  const o = { app: argv[0], harness: argv[1], effort: null, model: null, prompt: null, dryRun: false, rest: argv.slice(2) };
  const rest = argv.slice(2), words = [];
  for (let i = 0; i < rest.length; i++) {
    const x = rest[i];
    if (x === "--skill") throw new Error("--skill 옵션은 없어졌습니다. 스킬은 프로필의 설치 상태입니다: bench skill <app> on|off  (상태는 bench status)");
...
function run(o) {
  ...
  const skillOn = skillInstalled(o.app, a);
  let cmd, args = [], env = {}, cwd = D.work;
  if (o.harness === "claude") {
    cmd = "claude"; env.CLAUDE_CONFIG_DIR = D.claude;
    ...
    cmd = "codex"; env.CODEX_HOME = D.codex; env.HOME = env.CODEX_HOME;
...
// ---------- skill on/off ----------
function skillCmd(app, a, state) {
  if (state !== "on" && state !== "off") throw new Error(`사용법: bench skill ${app} on|off   (현재: ${skillInstalled(app, a) ? "설치됨" : "미설치"})`);
  if (!exists(dirs(app).claudeMcp)) throw new Error(`프로필이 없습니다: bench init ${app}`);
  setSkill(app, a, state === "on");
  console.log(`${app}: 스킬 ${state === "on" ? "설치됨" : "제거됨"} (${a.skills.map((s) => path.basename(s)).join(", ")}) — 세 하네스 프로필에 함께 적용`);
}
